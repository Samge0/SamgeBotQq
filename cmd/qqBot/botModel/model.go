...
type Sender struct {
	...
	Card  string // [群字段]群名片／备注
	Area  string // [群字段]地区
	Level string // [群字段]成员等级
	Role  string // [群字段]角色，owner 或 admin 或 member
	Title string // [群字段]专属头衔
}

func (sender *Sender) String() string {
	return fmt.Sprintf("%s[%s]", sender.Nickname, sender.UserID)
}

// MessageParam 用于参数传递的消息体
type MessageParam struct {
	Msg       *MessagePrivate
	GroupID   int64 // [群字段]群号
	Anonymous struct {
		Id   int64  // 匿名用户 ID
		Name string // 匿名用户名称
		Flag string // 匿名用户 flag，在调用禁言 API 时需要传入
	} //	[群字段]匿名信息，如果不是匿名消息则为 null
}
...
func (m *MessagePrivate) AsMessageParam() *MessageParam {
	return &MessageParam{Msg: m}
}

// IsGroup 是否为群组，实际上服务在群组发送过来的SubType值为normal，需要自己改为group
func (m *MessagePrivate) IsGroup() bool {
	return m.SubType == "group"
}
...
func (m *MessageGroup) AsMessageParam() *MessageParam {
	return &MessageParam{
		Msg:       m.Msg,
		GroupID:   m.GroupID,
